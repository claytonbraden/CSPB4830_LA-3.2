...
std::vector<std::string> Split(std::string whole, std::string separator) {
	...
	for(size_t i=0; i < whole.size(); i++) {
		...
		if(whole[i] == c) { //check if letter is seperator
			if(i == 0) {  //check if we are on the 1st letter
				current = "";
				returnVector.push_back(current);
			}
			
			else { //else just increment cursor and add current to the next vector position, use .append since it will be multicharachter
				if(i == (whole.size() - 1) ) { //if we are on last letter in string, dont add to vector (will do at end)
					continue;
				}
				else {
					returnVector.push_back(current); //else add the current word to vector
					current = "";
				}
			}
		}
		else {
				current.push_back(whole[i]);  //add letter to cursor
			}
	}
	returnVector.push_back(current);
...
std::string RemoveAllSubstrings(std::string s1, std::string s2) {
	if(s2.size() > s1.size()) { //return s1 if s2 is bigger (s2 cant be in s1)
		return s1;
	}
	else {
		size_t s2_currentLetter = 0;
		std::string new_s1_firstHalf;
		std::string new_s1_secondHalf;
		std::string new_s1 = s1;
		bool firstWordFound = false;
		for(size_t i = 0; i < s1.size(); i++) {
			if(s2[s2_currentLetter] == s1[i] ) { //check if letter is current
				if(s2_currentLetter != (s2.size() - 1) ) { //go on to next letter of s2 if we are not on last letter
					s2_currentLetter += 1;
					continue;
				}
				else {
					if(firstWordFound) { //check if we already have letters not in s1
						new_s1_firstHalf = s1.substr(0, i - s2.size()+1); //get first half of string (substring that does not include s2)
						new_s1_secondHalf = s1.substr(i+1, s1.size() ); //get second half of string (substring after s2)
						s1 = new_s1_firstHalf + new_s1_secondHalf; //set new s1 
						i -= (s2.size()); //set new i now that s1 was modified (decrease by number of letters in s2)
					}
					else { //if we are still on beginning of s1, we only want to take the last part of it
						new_s1_secondHalf = s1.substr(i+1, s2.size() );
						s1 = new_s1_secondHalf; //set new s1 
						i -= (s2.size());  //set new i now that s1 was modified (decrease by number of letters in s2)
					}
				}
			}
			else { //go to next letter, and set s2 cursor back to 0
				s2_currentLetter = 0;
				firstWordFound = true;
			}
		}
...
std::string Join(std::vector<std::string> pieces, std::string glue) {
	...
	for(size_t i = 0; i < pieces.size(); i++ ) {
		if(i == 0) {
			returnString = returnString + pieces[i];
		}
		else {
